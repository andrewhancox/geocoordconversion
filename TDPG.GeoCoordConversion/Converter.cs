...
namespace TDPG.GeoCoordConversion
{
    ...
    {
        private static double _NatGridScaleFactor = 0.9996012717; // NatGrid scale factor on central meridian
        private static GridReference _NatNETrueOrigin = new GridReference(400000,-100000 );
        private static PolarGeoCoordinate _NatGridTrueOrigin = PolarGeoCoordinate.ChangeUnits(
            new PolarGeoCoordinate(49, -2, 0, AngleUnit.Degrees, CoordinateSystems.OSGB36),
            AngleUnit.Radians);

        private static EllipseParameter _Airy1830 = EllipseParameter.GetEllipseParameters(EllipseParameter.Ellipses.Airy1830);
        private static double _e2 = 1 - Math.Pow(_Airy1830.SemiMinorAxis, 2) / Math.Pow(_Airy1830.SemimajorAxis, 2);
        private static double _n = (_Airy1830.SemimajorAxis - _Airy1830.SemiMinorAxis) / (_Airy1830.SemimajorAxis + _Airy1830.SemiMinorAxis);
        private static double _n2 = Math.Pow(_n, 2);
        private static double _n3 = Math.Pow(_n, 3);
        ...
        private static PolarGeoCoordinate Convert(PolarGeoCoordinate originalCoord, EllipseParameter e1, HelmertTransform t, EllipseParameter e2)
        {
